# -*- coding: utf-8 -*-
"""
Created on Mon Mar 19 21:55:51 2018
...
class doc_labeling(object):
    ...
    def get_body_ent(self,limit):
        ...
        cur.execute("SELECT genre FROM article1 WHERE genre LIKE '%entertainment%' LIMIT " + str(limit))
        rows = cur.fetchall()
        for k in rows:
            genre.append(k)
        
        cur.execute("SELECT body FROM article1 WHERE genre LIKE '%entertainment%' LIMIT " + str(limit))
        rows = cur.fetchall()
        for k in rows:
            body.append(k)
        #print(body)
    ...
    def doc_to_vec(self,documents):
        ...
        model.save('model_dailynews1_deepcut_ec')
